...
namespace Pat.Subscriber.MessageMapping
{
    public class MessageMapper
    {
        ...
        internal static void AddCustomMessageMaps(IList<CustomMessageTypeMap> customMessageTypeMap)
        {
            foreach (var map in customMessageTypeMap ?? new CustomMessageTypeMap[0])
            {
                _messageTypeMappings.Add(map.ToMessageTypeMapping());
            }
        }
    }
}